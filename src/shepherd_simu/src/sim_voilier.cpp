...
void cmdCallback(const shepherd_msg::SailboatCmd::ConstPtr &msg) {
    ...
    ROS_DEBUG("Boat commands : [%f] [%f]", msg->rudder_angle, msg->sail_angle);
}
...
//    worldEnv.wind_strength = msg->wind_strength;
    ...
    ROS_DEBUG("World parameters : [%f] [%f]", msg->wind_angle, msg->wind_strength);
}
...
int main(int argc, char **argv) {
    ...
    ROS_INFO("Sim speed factor (sim_voilier): [%f]", accelRate);
    ...
    ROS_INFO("Pose noise (sim_voilier): [%f]", pose_noise);


    double heading_noise;
    ros::param::param<double>("sailboat_imu_noise", heading_noise, 0.1);
    ROS_INFO("Heading noise (sim_voilier): [%f]", heading_noise);
